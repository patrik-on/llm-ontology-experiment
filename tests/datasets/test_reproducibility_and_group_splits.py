...
from llm_ontology.datasets.methods2test import require_project_disjoint_official_splits
from llm_ontology.datasets.ml4refactoring import split_records as split_ml4_records
...
def test_environment_lock_contains_all_model_revisions() -> None:
    payload = json.loads(
        Path("configs/shared/environment/rag_baseline_windows_cpu.lock.json").read_text(
            encoding="utf-8"
        )
    )
